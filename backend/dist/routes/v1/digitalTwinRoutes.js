 ...
 */
router.post('/retrain', verifyToken, async (_req, res) => {
    try {
 ...
 */
router.post('/nl-query', verifyToken, async (req, res) => {
    try {
 ...
 */
router.post('/cognitive-recall', verifyToken, async (req, res) => {
    try {
        const { topic, rating } = req.body;
        const result = await CognitiveLearningEngine.evaluateRecallPerformance(req.user._id.toString(), topic || 'Core Concept', rating ?? 4);
        return res.json({ success: true, data: result });
    }
    catch (err) {
        return res.status(500).json({ success: false, message: err.message });
