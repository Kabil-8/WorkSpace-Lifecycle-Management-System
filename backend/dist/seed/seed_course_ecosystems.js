...
import { CollegeCourse, CollegeCourseEnrollment, CollegeAssignment, CollegeQuiz, CollegeExam } from '../models/CollegeCourse.js';
import { VideoCourse, VideoQuiz } from '../models/VideoCourse.js';
dotenv.config();
...
export async function seedCourseEcosystems() {
    ...
    for (const cData of collegeCoursesData) {
        ...
        if (courseObj && students.length > 0) {
            for (const st of students) {
                await CollegeCourseEnrollment.findOneAndUpdate({ userId: st._id, collegeCourseId: courseObj._id }, {
                    userId: st._id,
        ...
        if (courseObj) {
            await CollegeAssignment.findOneAndUpdate({ collegeCourseId: courseObj._id, title: `Assignment 1: ${courseObj.title} Fundamentals` }, {
                collegeCourseId: courseObj._id,
            ...
            await CollegeQuiz.findOneAndUpdate({ collegeCourseId: courseObj._id, title: `${courseObj.courseCode} Unit 1 Assessment` }, {
                collegeCourseId: courseObj._id,
            ...
            await CollegeExam.findOneAndUpdate({ collegeCourseId: courseObj._id, title: `Midterm Examination 1 — ${courseObj.courseCode}` }, {
                collegeCourseId: courseObj._id,
    ...
    for (const vData of videoCoursesData) {
        ...
        if (vObj) {
            ...
            await VideoQuiz.findOneAndUpdate({ videoCourseId: vObj._id, lessonId: vObj.sections[0]?.lessons[0]?.lessonId || 'l1' }, {
