...
function randomDate(daysBack = 180, daysForward = 0) {
    const now = Date.now();
...
async function seed() {
    ...
    if (hodUser) {
        await Department.findOneAndUpdate({ code: 'CSE' }, { hodId: hodUser._id, hodName: hodUser.name });
    }
    // ═══════════════════════════════════════════════════════════════════════════
    ...
    const notifTypes = ['assignment', 'attendance', 'exam', 'placement', 'gamification', 'system', 'forum', 'event', 'general'];
