...
export class CognitiveLearningEngine {
     ...
     */
    static async evaluateRecallPerformance(userId, topic, quality) {
        try {
            ...
            if (result.correct)
                ...
                item.mistakeCount += 1;
            await item.save();
            logger.info({ userId, topic: topicId, quality: q, newInterval: result.newInterval, mastery: result.mastery }, '[CognitiveLearningEngine] SM-2 updated via RecallItem ✓');
            return {
