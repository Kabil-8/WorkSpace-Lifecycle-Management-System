        ...
        try {
            ...
            switch (functionName) {
                ...
                case 'evaluate_cognitive_recall': {
                    ...
                    const q = [1, 3, 5].includes(Number(args.rating)) ? Number(args.rating) : 3;
                ...
                case 'open_module': {
                    ...
                    if (raw.includes('interview') || raw.includes('mock'))
                        target = 'interview';
                    else if (raw.includes('attendance'))
                        target = 'attendance';
                    else if (raw.includes('quiz'))
                        target = 'quizzes';
                    else if (raw.includes('compiler') || raw.includes('code'))
                        target = 'compiler';
                    else if (raw.includes('job'))
                        target = 'jobs';
                    else if (raw.includes('resume'))
                        target = 'resume';
                    else if (raw.includes('placement'))
                        target = 'placement';
                    else if (raw.includes('course'))
                        target = 'courses';
                    else if (raw.includes('assignment'))
                        target = 'assignments';
                    else if (raw.includes('event'))
                        target = 'events';
                    else if (raw.includes('note'))
                        target = 'notes';
                    else if (raw.includes('timetable') || raw.includes('schedule'))
                        target = 'timetable';
                    else if (raw.includes('analytics'))
                        target = 'analytics';
                    else if (raw.includes('proctor') || raw.includes('exam'))
                        target = 'proctor/exams';
                    else if (raw.includes('forum'))
                        target = 'forum';
                    else if (raw.includes('workspace') || raw.includes('kanban'))
                        target = 'workspace';
                    else
                        target = raw.replace(/^\//, '') || 'dashboard';
                    result = {
                        success: true,
                        action: 'NAVIGATE',
                        target,
                ...
                case 'broadcast_announcement': {
                    ...
                    if (notifs.length > 0) {
                        if (session)
                            await Notification.insertMany(notifs, { session });
                        else
                ...
                case 'query_database': {
                    ...
                    if (col.includes('user')) {
                        ...
                        const cList = collegeCourses.map((c) => `- 🎓 **${c.courseCode}: ${c.title}** (${c.department} Sem ${c.semester})`).join('\n');
                        const vList = videoCourses.map((v) => `- 🌐 **${v.title}** (${v.category} — ${v.durationHours}h)`).join('\n');
                        ...
                        const list = events.map((e) => `- **${e.title}** (${e.category || e.type || 'Campus'}) — Date: ${e.date ? new Date(e.date).toDateString() : 'TBD'}`).join('\n');
