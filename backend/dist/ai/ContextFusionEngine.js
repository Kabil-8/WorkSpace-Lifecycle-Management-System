...
export class ContextFusionEngine {
     ...
     */
    static computeFreshness(date, tauDays = 14) {
        if (!date)
            return 0.5;
    ...
    static classifyIntent(query) {
        ...
        if (q.includes('dynamic programming') || q.includes('dp'))
            entities.topic = 'Dynamic Programming';
        else if (q.includes('recursion'))
            entities.topic = 'Recursion';
        else if (q.includes('binary search') || q.includes('tree') || q.includes('trie'))
            entities.topic = 'Trees';
        else if (q.includes('operating systems') || q.includes('os'))
            entities.topic = 'Operating Systems';
        else if (q.includes('dbms') || q.includes('database'))
            entities.topic = 'Database Systems';
        else if (q.includes('graph theory') || q.includes('dijkstra'))
            entities.topic = 'Graph Theory';
        else if (q.includes('concurrency') || q.includes('multithreading'))
            entities.topic = 'Concurrency';
        const numMatch = q.match(/(\d+(?:\.\d+)?)\s*%/i) || q.match(/attendance.*?(\d+)/i);
        if (numMatch)
            entities.percentage = parseFloat(numMatch[1]);
     ...
     */
    static planSources(intentType, query) {
     ...
     */
    static async fuseContext(userId, query, options = {}) {
        ...
        const plan = { ...basePlan, ...(options.forcePlan || {}) };
        // 3. Retrieve Student Profile & Academic Record
        let profileData = null;
        let academicAttendance = 0;
        let userCgpa = 8.5;
        ...
        try {
            ...
            if (u) {
                ...
                userSem = u.semester || userSem;
                userCgpa = u.cgpa || userCgpa;
                academicAttendance = u.attendanceRate || 0;
        ...
        if (plan.useAcademic || plan.useTelemetry) {
            try {
                ...
                if (attRecord && attRecord.overallPercentage) {
                    academicAttendance = attRecord.overallPercentage;
                    sourcesUsed.push('StudentAttendance');
                    evidenceSummary.push({
                        source: 'StudentAttendance',
                        relevance: 0.98,
                        freshness: this.computeFreshness(attRecord.updatedAt)
                    });
                }
            }
            catch { }
            ...
            try {
                ...
                if (groundedContext && groundedContext.hasInstitutionalEvidence) {
                    ragResult = {
                        ...
                        citations: (groundedContext.sourceCitations || []).map((s) => `${s.title} [${s.section}]`),
