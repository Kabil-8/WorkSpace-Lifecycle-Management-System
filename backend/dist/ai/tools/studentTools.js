...
export const studentTools = [
    {
        ...
        execute: async (_args, userContext) => {
            ...
            if (!u)
                return { hasData: false, status: 'INSUFFICIENT_DATA', message: 'No student profile record found.' };
            return {
                hasData: true,
                userName: u.name,
                email: u.email,
        ...
        execute: async (_args, userContext) => {
            try {
                ...
                return {
                    ...
                    assignments: assignments.map((a) => ({ title: a.title, dueDate: a.dueDate, courseName: a.courseName || a.department })),
                };
            }
            catch (err) {
