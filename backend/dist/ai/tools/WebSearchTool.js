...
export class WebSearchTool {
    ...
    static isSafeUrl(urlStr) {
        try {
            ...
            if (/^10\./.test(host) || /^192\.168\./.test(host) || /^172\.(1[6-9]|2[0-9]|3[0-1])\./.test(host))
                return false;
            if (/^169\.254\./.test(host) || /^100\.64\./.test(host))
                return false; // link-local + CGNAT
            return true;
        }
        catch {
            return false;
            ...
            if (provider === 'serper') {
                ...
                if (res.ok) {
                    ...
                    const results = (data.organic || [])
                        .filter((r) => this.isSafeUrl(r.link))
                        .map((r) => ({
    ...
    static async openWebPage(url) {
        ...
        try {
            ...
            if (!res.ok)
                return { success: false, error: `Page returned HTTP ${res.status}` };
     ...
     */
    static async followRelevantLink(pageUrl, query) {
        try {
