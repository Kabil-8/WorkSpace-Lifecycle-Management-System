        ...
        execute: async (_args, userContext) => {
            try {
                ...
                const parentUser = await User.findById(userContext.userId)
                    ...
                    .lean();
                const linkedId = parentUser?.linkedStudentId
                    || (parentUser?.linkedStudentIds || [])[0];
                if (!linkedId) {
                    return {
                        hasData: false,
    ...
    },
    {
        ...
        execute: async (_args, userContext) => {
            try {
                ...
                const linkedId = parentUser?.linkedStudentId || (parentUser?.linkedStudentIds || [])[0];
                if (!linkedId) {
                ...
                const withStatus = assignments.map((a) => {
                    const submission = (a.submissions || []).find((s) => s.studentId?.toString() === linkedId.toString());
                    return {
                        title: a.title,
                        dueDate: a.dueDate,
                        maxMarks: a.maxMarks,
