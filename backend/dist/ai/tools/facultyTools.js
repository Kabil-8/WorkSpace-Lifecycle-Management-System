...
export const facultyTools = [
    ...
    },
    {
        ...
        execute: async (args, userContext) => {
            try {
                const filter = { instructorId: userContext.userId };
                if (args?.assignmentId)
                    filter._id = args.assignmentId;
                const assignments = await Assignment.find(filter).lean();
                const pendingList = [];
                for (const a of assignments) {
                    const ungraded = (a.submissions || []).filter((s) => s.grade === undefined || s.grade === null);
                    if (ungraded.length > 0) {
                        pendingList.push({
                            ...
                            ungradedStudents: ungraded.map((s) => ({ name: s.studentName, submittedAt: s.submittedAt })),
                        });
                    }
                }
                return {
    ...
    },
    {
        ...
        execute: async (args, userContext) => {
            try {
                const filter = {};
                if (args?.courseId)
                    filter.courseId = args.courseId;
                if (!args?.courseId && userContext.department)
                    filter.department = userContext.department;
                ...
                for (const r of records) {
                    ...
                    if (!byStudent[sid])
                        byStudent[sid] = { present: 0, total: 0, name: r.studentName };
                    byStudent[sid].total++;
                    if (r.status === 'Present' || r.status === 'present')
                        byStudent[sid].present++;
                }
