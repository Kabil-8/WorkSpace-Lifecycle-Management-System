...
export class ProactiveInterventionEngine {
    ...
    static async analyzeAndIntervene(userId) {
        ...
        for (const evt of recentEvents) {
            const t = evt.topicId || evt.subject;
            if (!t || topicMastery[t])
                continue; // Already processed newest event for this topic
        ...
        let weakTopics = Object.keys(topicMastery).filter(t => topicMastery[t].status === 'weak');
