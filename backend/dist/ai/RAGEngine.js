...
export class RAGEngine {
    ...
    static async generateEmbedding(text) {
        try {
            ...
            const res = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/text-embedding-004:embedContent?key=${key.trim()}`, {
                method: 'POST',
     ...
     */
    static async retrieveChunks(query, department) {
     ...
     */
    static async chunkAndEmbedDocument(title, category, department, content, uploadedBy) {
    ...
    static splitTextIntoChunks(text, chunkSize = 500, overlap = 100) {
    ...
    static generateDeterministicVector(text) {
    ...
    static async seedDefaultKnowledge() {
        try {
            ...
            if (count > 0)
                return;
            await RAGEngine.chunkAndEmbedDocument('EduSphere Academic & Attendance Regulations 2026', 'university_rules', 'All Departments', 'Attendance Rule 75%: Students must maintain a minimum of 75% attendance in each subject to be eligible for end-semester examinations. Medical leave requests must be submitted within 3 days of absence. A maximum of 10% condonation is allowed upon HoD approval.');
            await RAGEngine.chunkAndEmbedDocument('Computer Science & Engineering Syllabus 2026', 'syllabus', 'Computer Science & Engineering', 'CSE Core Modules: Data Structures & Algorithms (Java/C), Database Management Systems (MongoDB/PostgreSQL), Systems Programming (C/POSIX), Web Engineering (React/Node.js), Machine Learning & AI.');
            await RAGEngine.chunkAndEmbedDocument('Campus Placement & Internship Policy 2026', 'placement_policy', 'Career & Placement', 'Placement Eligibility: Minimum 6.5 CGPA with zero active backlogs. Students can hold up to 1 Dream Offer (> ₹10 LPA). ATS resume verification is mandatory prior to campus interviews.');
