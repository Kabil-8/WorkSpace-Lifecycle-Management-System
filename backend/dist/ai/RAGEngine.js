...
import { RerankingService } from './rag/RerankingService.js';
...
export class RAGEngine {
    ...
    static async retrieveChunks(query, department) {
        ...
        try {
            const studentContext = { department };
            ...
            return reranked.map(c => `[Doc: ${c.docTitle || c.documentTitle} | Section: ${c.sectionTitle} | Relevance: ${(c.finalScore * 100).toFixed(1)}%]\n${c.text}`);
        }
        catch (err) {
     ...
     */
    static async retrieveGroundedContext(query, studentContext) {
        try {
            const candidates = await RetrievalService.retrieveCandidateChunks(query, studentContext, 8);
            const reranked = RerankingService.rerank(query, candidates, studentContext, 4);
     ...
     */
    static async chunkAndEmbedDocument(title, category, department, content, uploadedBy, semester) {
        const ingestedDoc = {
    ...
    static async seedDefaultKnowledge() {
        try {
            ...
            if (ingestedDocs.length === 0) {
                ...
                await RAGEngine.chunkAndEmbedDocument('EduSphere Academic & Attendance Regulations 2026', 'attendance_policy', 'All Departments', 'Section 1. Mandatory Attendance: 75% minimum attendance required. Section 2. Medical Condonation: Shortage between 65% and 75% may be condoned on medical grounds with HoD approval within 3 working days.', null);
                return;
            }
