...
export class PlanningEngine {
     ...
     */
    static createPlan(query, role, userContext) {
        const q = query.toLowerCase();
        const allowedToolObjects = PermissionLayer.getAllowedTools(role);
        const allowedToolNames = allowedToolObjects[0]?.functionDeclarations?.map((t) => t.name) || [];
        const steps = [];
    ...
    static evaluateResponseQuality(responseText, contextData) {
