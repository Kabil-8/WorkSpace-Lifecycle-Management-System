...
export class AdminService {
    static async getContextData(userId) {
        try {
            ...
            const [totalUsers, activeStudents, activeFaculty, todayAttendance, departments, recentAuditLogs, recentAlerts, notifications,] = await Promise.all([
            ...
            return {
                ...
                recentAuditAlerts: recentAuditLogs.map((log) => ({
                    action: log.action,
                ...
                criticalAlerts: recentAlerts.map((log) => ({
                    action: log.action,
                    description: log.description,
                    timestamp: log.createdAt,
                })),
