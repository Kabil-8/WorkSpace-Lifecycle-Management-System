...
async function runAcademicResultE2ETest() {
    ...
    try {
        ...
        const studentA = await User.findOneAndUpdate({ email: 'gradebook_student_a@edusphere.ai' }, {
        ...
        const studentB = await User.findOneAndUpdate({ email: 'gradebook_student_b@edusphere.ai' }, {
        ...
        const faculty = await User.findOneAndUpdate({ email: 'gradebook_faculty@edusphere.ai' }, {
        ...
        const mockRes = { status: () => mockRes, json: (d) => d };
        ...
        const sem5Result = await AcademicResult.findOne({ studentId: studentA._id, semester: 5 }).lean();
        ...
        const edenToolRes = await ToolRegistry.executeTool('get_my_transcript', {}, { userId: studentA._id.toString(), role: 'student', userName: studentA.name });
        ...
        const securityRes = await AcademicResultController.getStudentTranscript({
            ...
            status: (code) => { if (code === 403)
                is403 = true; return mockRes; },
            json: (d) => d
        });
        if (is403) {
