...
export class AttendanceController {
    ...
    static async bulkMarkAttendance(req, res) {
        try {
            ...
            const logs = await Promise.all((attendanceList || []).map(async (item) => {
    ...
    static async getClassSheet(req, res) {
        try {
            const user = req.user;
            ...
            const classList = await Promise.all(students.map(async (s, idx) => {
                ...
                const latestLog = await StudentAttendance.findOne({ studentId: s._id }).sort({ date: -1 }).lean();
                return {
                    id: s._id.toString(),
    ...
    static async getFacultyDailyLogs(req, res) {
        try {
    ...
    static async getLeaveRequests(req, res) {
        try {
            ...
            if (user?.role === 'student')
                ...
                filter.$or = [{ applicantId: user._id }, { role: 'student' }];
    ...
    static async reviewLeave(req, res) {
        try {
            ...
            const user = req.user;
            const updated = await LeaveRequest.findByIdAndUpdate(id, { status, comments, reviewedBy: user?.name || 'Faculty Admin' }, { new: true });
    ...
    static async getStudentAnalytics(req, res) {
        try {
            ...
            const attendedClasses = logs.filter((l) => l.status === 'Present' || l.status === 'present').length;
            const lateCount = logs.filter((l) => l.status === 'Late').length;
            const leaveCount = logs.filter((l) => l.status === 'Medical Leave' || l.status === 'Excused').length;
            ...
            logs.forEach((l) => {
                ...
                if (l.status === 'Present' || l.status === 'present')
    ...
    static async queryAttendance(req, res) {
        try {
