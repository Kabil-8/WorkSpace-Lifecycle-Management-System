...
export class ProctorController {
    ...
    static async createExam(req, res) {
        try {
            ...
            const newExam = await Exam.create({
                ...
                totalPoints: (questions || []).reduce((acc, q) => acc + (q.points || 5), 0) || 100,
                createdByName: req.user?.name || 'Faculty Instructor',
    ...
    static async startExam(req, res) {
        try {
            ...
            const attempt = await StudentAttempt.create({
                examId,
                studentId: studentId || req.user?._id || '650000000000000000000001',
                studentName: studentName || req.user?.name || 'Student Candidate',
                studentEmail: studentEmail || req.user?.email || 'student@edusphere.edu',
                status: 'in_progress',
    ...
    static async logEyeTracking(req, res) {
        try {
            const { attemptId, gazeDirection, irisRatio, eyeFocusScore } = req.body;
            await ProctorLog.findOneAndUpdate({ attemptId }, {
                $push: {
                    eyeTrackingLogs: {
    ...
    static async logHeadPose(req, res) {
        try {
    ...
    static async logBrowserEvents(req, res) {
        try {
    ...
    static async issueWarning(req, res) {
        try {
            ...
            if (newIntegrityScore <= 20)
                riskCategory = 'High Risk';
            else if (newIntegrityScore <= 40)
                riskCategory = 'Suspicious';
            else if (newIntegrityScore <= 60)
                riskCategory = 'Moderate Risk';
            else if (newIntegrityScore <= 80)
    ...
    static async getIntegrityReport(req, res) {
        try {
    ...
    static async getFacultyLiveMonitor(req, res) {
        try {
    ...
    static async getFacultyAnalytics(req, res) {
        try {
