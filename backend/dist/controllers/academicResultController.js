...
const DEFAULT_SEMESTER_CURRICULUM = {
    1: [
...
export class AcademicResultController {
     ...
     */
    static calculateGrade(totalMarks) {
        if (totalMarks >= 90)
            return { grade: 'S', gradePoints: 10 };
        if (totalMarks >= 80)
            return { grade: 'A', gradePoints: 9 };
        if (totalMarks >= 70)
            return { grade: 'B', gradePoints: 8 };
        if (totalMarks >= 60)
            return { grade: 'C', gradePoints: 7 };
        if (totalMarks >= 50)
            return { grade: 'D', gradePoints: 6 };
     ...
     */
    static async getCoursesBySemester(req, res) {
        try {
    ...
    static async updateCourse(req, res) {
        try {
            ...
            const updated = await CollegeCourse.findByIdAndUpdate(id, {
     ...
     */
    static async getGradebookSheet(req, res) {
        try {
            const semester = Number(req.query.semester) || 5;
            const rawCode = req.query.courseCode || 'CS501';
            ...
            const roster = students.map((s, idx) => {
                ...
                const gradeEntry = studentResult?.courseGrades?.find((cg) => cg.courseCode.toUpperCase() === courseCode.toUpperCase());
                ...
                return {
                    ...
                    rollNo: s.rollNumber || `CS2021${String(idx + 1).padStart(3, '0')}`,
                    studentName: s.name,
                    photo: s.avatarUrl || `https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150`,
                    internalMarks,
                    endSemMarks,
                    totalMarks,
     ...
     */
    static async bulkRecordCourseGrades(req, res) {
        try {
            ...
            await Promise.all(grades.map(async (g) => {
                ...
                if (!semResult) {
                    semResult = new AcademicResult({
                        ...
                        rollNo: targetStudent.rollNumber || 'N/A',
                    ...
                    const existingIdx = semResult.courseGrades.findIndex((cg) => cg.courseCode.toUpperCase() === cleanCourseCode.toUpperCase());
                    if (existingIdx >= 0) {
                    ...
                    const totalPoints = semResult.courseGrades.reduce((sum, cg) => sum + cg.gradePoints * cg.credits, 0);
                    const totalSemCredits = semResult.courseGrades.reduce((sum, cg) => sum + cg.credits, 0);
                    ...
                    semResult.status = semResult.courseGrades.some((cg) => cg.grade === 'F') ? 'failed' : 'passed';
                }
                await semResult.save();
                // Sync cumulative CGPA to User model
                const allSemResults = await AcademicResult.find({ studentId }).lean();
                const totalPointsAll = allSemResults.reduce((sum, r) => sum + r.sgpa * r.totalCreditsEarned, 0);
                const totalCreditsAll = allSemResults.reduce((sum, r) => sum + r.totalCreditsEarned, 0);
     ...
     */
    static async ensureStudentAcademicResults(studentId) {
        try {
            ...
            const rollNo = targetStudent.rollNumber || 'CS2021001';
    ...
    static async getStudentTranscript(req, res) {
        try {
            ...
            const semesters = results.map(r => {
                const semCredits = (r.courseGrades || []).reduce((sum, g) => sum + (g.credits || 4), 0);
            ...
            return res.json({
                ...
                data: {
                    ...
                    rollNo: results[0]?.rollNo || user?.rollNumber || 'CS2021001',
     ...
     */
    static async recordCourseGrade(req, res) {
        try {
            ...
            if (!semResult) {
                ...
                const existingIdx = semResult.courseGrades.findIndex((g) => g.courseCode === courseCode);
                if (existingIdx >= 0) {
                    semResult.courseGrades[existingIdx] = newCourseGrade;
                }
                else {
                ...
                const totalPoints = semResult.courseGrades.reduce((sum, g) => sum + g.gradePoints * g.credits, 0);
                const totalSemCredits = semResult.courseGrades.reduce((sum, g) => sum + g.credits, 0);
                semResult.sgpa = totalSemCredits > 0 ? Number((totalPoints / totalSemCredits).toFixed(2)) : 0;
                semResult.totalCreditsEarned = totalSemCredits;
                semResult.status = semResult.courseGrades.some((g) => g.grade === 'F') ? 'failed' : 'passed';
            }
            await semResult.save();
            const allSemResults = await AcademicResult.find({ studentId }).lean();
            const totalPointsAll = allSemResults.reduce((sum, r) => sum + r.sgpa * r.totalCreditsEarned, 0);
