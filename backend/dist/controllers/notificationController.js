...
async function ensureDataDrivenNotifications(user) {
    try {
...
export class NotificationController {
    static async getMyNotifications(req, res) {
        try {
            ...
            if (unreadOnly !== 'true')
                await RedisCache.set(cacheKey, result, CacheTTL.notifications);
    ...
    static async markRead(req, res) {
        try {
            ...
            if (Array.isArray(notificationIds) && notificationIds.length > 0) {
                await Notification.updateMany({ _id: { $in: notificationIds }, userId: req.user._id }, { isRead: true, readAt: new Date() });
            }
