...
export class HODController {
     ...
     */
    static async getDepartmentAttendanceStats(req, res) {
        try {
            const dept = req.user.department || req.query.department;
            ...
            for (const student of students) {
                ...
                stats.push({ name: student.name, rollNumber: student.rollNumber, semester: student.semester, attendancePct: pct, isAtRisk: pct < 75 });
            }
     ...
     */
    static async getDepartmentPerformanceStats(req, res) {
        try {
            const dept = req.user.department || req.query.department;
            const students = await User.find({ role: 'student', department: dept, isActive: true })
