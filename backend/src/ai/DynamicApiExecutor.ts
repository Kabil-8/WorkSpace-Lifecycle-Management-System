...
export class DynamicApiExecutor {
   ...
   */
  static async execute(functionName: string, args: any, userContext?: any): Promise<any> {
    try {
      ...
      if (functionName === 'query_project_model') {
        ...
        const formattedList = records
          .map((r: any) => `- **${r.title || r.name || r.code || r._id}** ${r.instructor ? `(Instructor: ${r.instructor})` : ''} ${r.role ? `[${r.role.toUpperCase()}]` : ''}`)
          .join('\n')
