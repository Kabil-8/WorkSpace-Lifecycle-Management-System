...
export class MultiAgentRouter {
  static determineAgent(query: string, role: string): AgentRole {
    const q = query.toLowerCase()

    // Inspect ONLY the active query to prevent context bleed from previous session topics
    if (q.includes('code') || q.includes('debug') || q.includes('java') || q.includes('python') || q.includes('compiler') || q.includes('algorithm') || q.includes('function') || q.includes('syntax') || q.includes('script')) return 'coding'
    if (q.includes('job') || q.includes('placement') || q.includes('resume') || q.includes('ats') || q.includes('interview')) return 'placement'
    if (q.includes('attendance') || q.includes('shortage') || q.includes('eligible')) return 'attendance'
    if (q.includes('quiz') || q.includes('exam') || q.includes('test') || q.includes('question paper')) return 'quiz'
    if (q.includes('assignment') || q.includes('homework') || q.includes('submission')) return 'assignment'
    if (q.includes('schedule') || q.includes('timetable') || q.includes('calendar') || q.includes('event')) return 'scheduling'
    if (q.includes('research') || q.includes('paper') || q.includes('thesis') || q.includes('citation')) return 'research'
    if (q.includes('document') || q.includes('pdf') || q.includes('docx') || q.includes('notes')) return 'document'
    if (q.includes('analytics') || q.includes('report') || q.includes('metrics') || q.includes('gpa')) return 'analytics'
    if (q.includes('notify') || q.includes('announcement') || q.includes('alert')) return 'notification'
    if (q.includes('plan') || q.includes('roadmap') || q.includes('strategy') || q.includes('workflow')) return 'planner'

    if (role === 'admin' || role === 'super_admin' || role === 'hod') return 'administration'
  ...
  static getAgentPrompt(agent: AgentRole): string {
