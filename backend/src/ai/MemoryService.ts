...
export class MemoryService {
   ...
   */
  static async getHistory(userId: string, role: string = 'student') {
    try {
   ...
   */
  static async addMessage(userId: string, sender: 'user' | 'eden', text: string, role: string = 'student') {
    try {
   ...
   */
  static async extractAndSaveFactsSemantically(userId: string, text: string, role: string) {
    try {
      const key = process.env.GEMINI_API_KEY || process.env.LLM_API_KEY
      if (!key || key.trim().length < 15) return
...
User message: "${text}"`
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/gemini-3.1-flash-lite:generateContent?key=${key.trim()}`,
        {
          method: 'POST',
      ...
      for (const fact of facts) {
        if (fact && typeof fact === 'string' && !memory.facts.includes(fact)) {
          ...
          if (!memory.factEmbeddings) memory.factEmbeddings = []
   ...
   */
  static async getLongTermMemories(userId: string, currentQuery?: string): Promise<string[]> {
    try {
      const mem = await EdenMemory.findOne({ userId }).lean()
      if (!mem) return []

      const facts: string[] = []
      if (mem.careerGoal) facts.push(`Career Goal: ${mem.careerGoal}`)
      if (mem.preferredLanguages?.length) facts.push(`Preferred Tech Stack: ${mem.preferredLanguages.join(', ')}`)
      ...
      if (currentQuery && mem.factEmbeddings && mem.factEmbeddings.length > 0) {
        ...
        const scoredFacts = mem.factEmbeddings.map((f: any) => {
  ...
  static formatForGemini(messages: any[]) {
  ...
  static async clearHistory(userId: string, role: string = 'student') {
