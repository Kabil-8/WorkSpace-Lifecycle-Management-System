...
export interface IntentResult {
  intent: EdenIntent
  agentRole: 'general' | 'coding' | 'debugging' | 'assignment' | 'course' | 'career' | 'telemetry'
...
export class IntentClassifier {
   ...
   */
  static classify(userQuery: string, pageContext?: any): IntentResult {
    const q = (userQuery || '').toLowerCase().trim()

    // Detect language if mentioned
    let language: string | undefined = undefined
    if (/\b(java)\b/i.test(q)) language = 'java'
    else if (/\b(python|py)\b/i.test(q)) language = 'python'
    else if (/\b(javascript|js|node|nodejs)\b/i.test(q)) language = 'javascript'
    else if (/\b(typescript|ts)\b/i.test(q)) language = 'typescript'
    else if (/\b(c\+\+|cpp)\b/i.test(q)) language = 'cpp'
    else if (/\b(c)\b/i.test(q) && !/\b(css)\b/i.test(q)) language = 'c'
    else if (/\b(html|css)\b/i.test(q)) language = 'html'
    else if (/\b(sql|postgres|mongo|mongodb)\b/i.test(q)) language = 'sql'

    // 1. Personal Data Telemetry (Attendance, Assignments, Digital Twin, Recall)
    if (
      q.includes('my attendance') || q.includes('attendance status') || q.includes('what is my attendance') ||
    ...
    if (
      q.includes('nullpointerexception') || q.includes('error') || q.includes('exception') ||
    ...
    if (
      q.includes('write a') || q.includes('write java') || q.includes('give java program') ||
    ...
    if (
      q.startsWith('describe') || q.startsWith('what is') || q.startsWith('explain') ||
