...
import { SourceManager, WebSource } from './SourceManager.js'
...
export class WebResearchAgent {
   ...
   */
  static isResearchNeeded(query: string): boolean {
   ...
   */
  private static generateSearchQueries(originalQuery: string): string[] {
   ...
   */
  private static rankSources(sources: WebSource[]): WebSource[] {
   ...
   */
  private static deduplicateSources(sources: WebSource[]): WebSource[] {
   ...
   */
  static async conductResearch(query: string, sourceManager: SourceManager): Promise<{
