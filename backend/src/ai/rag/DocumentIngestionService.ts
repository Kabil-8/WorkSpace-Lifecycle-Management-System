...
export class DocumentIngestionService {
   ...
   */
  static async ingestDirectory(dirPath: string): Promise<IngestedDocument[]> {
   ...
   */
  static parseDocumentContent(raw: string, filePath: string): IngestedDocument {
    ...
    if (deptMatch) {
      ...
      if (/computer science|cse/i.test(deptStr)) department = 'Computer Science & Engineering'
      else if (/all/i.test(deptStr) || /graduating|final year|engineering|b\.tech/i.test(deptStr)) department = 'All Departments'
      else department = deptStr
    }
    ...
    if (lowerPath.includes('attendance')) category = 'attendance_policy'
    else if (lowerPath.includes('exam')) category = 'examination_rules'
    else if (lowerPath.includes('placement')) category = 'placement_policy'
    else if (lowerPath.includes('syllabus') || lowerPath.includes('curriculum')) category = 'syllabus'
    else if (lowerPath.includes('regulation')) category = 'academic_regulations'
  ...
  private static collectFilesRecursive(dir: string): string[] {
    let files: string[] = []
