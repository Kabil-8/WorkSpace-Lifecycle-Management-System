...
export class EdenAutonomousAgent {
  ...
  static async attendanceRiskAlert() {
    ...
    try {
      ...
      for (const student of students) {
        ...
        if (total < 5) continue // Not enough records

        const pct = Math.round((present / total) * 100)
        if (pct >= 75) continue // Not at risk
        ...
        const message = await EdenAutonomousAgent.generateAlertMessage(
          `Student ${student.name} has ${pct}% attendance (${present}/${total} classes). They need ${sessionsNeeded} more consecutive attended classes to reach 75%. Write a short, empathetic but urgent 2-sentence notification warning them.`,
          `⚠️ Attendance Alert: Your attendance is ${pct}% — below the 75% threshold. You need to attend ${sessionsNeeded} more classes to become eligible for exams.`,
        )
  ...
  static async placementReadinessCheck() {
    ...
    try {
      ...
      for (const student of students) {
        ...
        if (readiness >= 70) continue // Already good
   ...
   */
  static async generateAlertMessage(prompt: string, fallback: string): Promise<string> {
    try {
      const key = process.env.GEMINI_API_KEY || process.env.LLM_API_KEY
      if (!key || key.length < 10) return fallback

      const res = await GeminiService.chat(
        'You are EDEN, the AI OS of EduSphere. Write concise, empathetic notification messages. Keep under 2 sentences.',
        prompt,
        [],
        'system',
      )
