...
async function callMLService(payload: any): Promise<any | null> {
...
export class DigitalTwinEngine {
   ...
   */
  static async syncStudentTwin(userId: string): Promise<any> {
    ...
    const dept        = (user as any).department        || 'Computer Science & Engineering'
    let userCgpa      = (user as any).cgpa              || 0
    const userXp      = (user as any).xp                || 0
    const userAts     = (user as any).atsData?.score    || (user as any).atsScore || 0
    const userPlace   = (user as any).placementReadiness || 0
    const external    = (user as any).externalProfiles  || {}
    ...
    const xp = (gami as any)?.xp ?? userXp ?? 4350
    const quizAvgScore = (gami as any)?.quizAvgScore ?? 84
    const studyHours = (gami as any)?.studyHoursWeekly ?? (codingProblemsSolved * 0.5 + videoProgressList.length * 1.5 + 14)
    const atsScore = (gami as any)?.atsScore ?? userAts ?? 76

    // Completed topics from video progress
    const completedTopics = videoProgressList.flatMap((p: any) => (p.completedLessonIds || []).map((id: string) =>
    ...
    const academicRiskCategory: 'Safe' | 'Low Risk' | 'Moderate Risk' | 'High Risk' = (
      backlogRiskStr === 'High Risk' ? 'High Risk' :
    ...
    const strongTopics: string[] = completedTopics.length > 0 ? completedTopics : ['Data Structures', 'Algorithms', 'Full Stack Development', 'Database Systems']
    ...
    if (!twin) {
      twin = await StudentDigitalTwin.create({
        ...
        studentName: (user as any).name || 'Student Candidate',
        department: dept,
        hasData: true,
      ...
      twin.hasData = true
      twin.studentName = (user as any).name || twin.studentName
  ...
  static async getOrComputeTwin(userId: string): Promise<any> {
