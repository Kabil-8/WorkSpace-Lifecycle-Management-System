...
export interface PredictiveRiskReport {
  attendanceRiskScore: number // 0 - 100 (higher means at risk)
...
export class PredictiveAIService {
   ...
   */
  static async analyzeStudentRisk(studentId: string): Promise<PredictiveRiskReport> {
    try {
      ...
      if (currentPct < 75) recommendations.push(`Attend ${Math.ceil(0.75 * totalAtt) - presentAtt} consecutive classes to reach exam eligibility limit.`)
      if (placementProbabilityPct < 70) recommendations.push('Complete 2 additional coding challenges to raise placement readiness.')
