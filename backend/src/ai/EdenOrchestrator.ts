...
export interface EdenOrchestratorRequest {
  userQuery: string
  userId: string
  userName?: string
  role?: string
...
export interface EdenOrchestratorResponse {
  success: boolean
  conversationId: string
  intent: string
  agentRole: string
...
export class EdenOrchestrator {
  static async processRequest(req: EdenOrchestratorRequest): Promise<EdenOrchestratorResponse> {
    ...
    const contextData = await ContextBuilder.buildContext(
      req.userId,
      req.userName || 'Student',
      role,
      cleanQuery,
      req.options?.pageRoute || '',
      req,
    )
    ...
    try {
      const studentContext = {
        ...
        cgpa: (contextData.user as any)?.cgpa
      }
      ...
      if (groundedContextPayload && groundedContextPayload.hasInstitutionalEvidence) {
        ...
        groundedContextPayload.sourceCitations.forEach((src: any) => {
    ...
    try {
      ...
      EdenConversation.findOneAndUpdate(
   ...
   */
  private static async persistRequestLog(data: {
    userId: string
    userRole: string
    query: string
    intent: string
