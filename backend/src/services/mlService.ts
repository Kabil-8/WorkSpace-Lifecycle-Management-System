...
export interface ATSResult {
  score: number // 0 - 100
  rating: 'Poor' | 'Needs Improvement' | 'Good' | 'Excellent' | 'Top Tier'
  breakdown: {
    keywordScore: number // max 30
    impactScore: number // max 25
    completenessScore: number // max 20
    formatScore: number // max 15
    skillRelevanceScore: number // max 10
  }
  missingKeywords: string[]
  suggestedActionVerbs: string[]
  formattingTips: string[]
  strengths: string[]
  improvements: Array<{ category: string; tip: string; impact: '+5 pts' | '+8 pts' | '+12 pts' }>
  issues?: ATSIssue[]
}
...
export interface InterviewQuestion {
  ...
  company: string
  role: string
  category: 'HR' | 'Technical' | 'System Design' | 'Behavioral' | 'Resume Proctored Technical' | 'Technical HR' | 'General HR'
...
export interface AnswerEvaluation {
  ...
  technicalClarityScore: number // 0 - 100
  starFormatScore: number // 0 - 100
...
export class MLService {
  public static calculateATSScore(data: ResumeData): ATSResult {
    ...
    const matchedTech = targetKeywords.filter(kw =>
    ...
    const matchedVerbs = POWER_ACTION_VERBS.filter(verb =>
      fullText.toLowerCase().includes(verb.toLowerCase())
    )
    ...
    if (hasSummary || fullText.length > 100) completenessScore += 5
    if (hasExperience) completenessScore += 5
    if (hasEducation) completenessScore += 5
    completenessScore = Math.min(20, Math.max(12, completenessScore))

    // 4. Formatting & Length (Max 15)
    let formatScore = 15
    if (fullText.length < 150) formatScore = 5
    else if (fullText.length < 300) formatScore = 10
    ...
    let rating: ATSResult['rating'] = 'Needs Improvement'
    if (totalScore >= 90) rating = 'Top Tier'
    else if (totalScore >= 80) rating = 'Excellent'
    else if (totalScore >= 70) rating = 'Good'
    else if (totalScore < 50) rating = 'Poor'

    const strengths: string[] = []
    if (matchedTech.length >= 5) strengths.push(`Strong technical stack detected (${matchedTech.slice(0, 4).join(', ')})`)
    if (hasNumbers) strengths.push('Quantified achievements present (percentages/metrics)')
    if (completenessScore === 20) strengths.push('Complete structural breakdown across contact, summary, education, and projects')
  ...
  public static getInterviewQuestions(company: string, role: string): InterviewQuestion[] {
    return [
      {
        id: 'q1',
        company,
  ...
  public static evaluateAnswer(question: InterviewQuestion, answer: string): AnswerEvaluation {
    ...
    if (overallScore >= 85) feedback += 'Excellent answer with deep technical terminology and STAR format structure.'
    else if (overallScore >= 70) feedback += `Solid response. Try adding more specific keywords like: ${missingKeywords.join(', ')}.`
    else feedback += 'Answer was brief. Expand using the STAR method (Situation, Task, Action, Result).'
