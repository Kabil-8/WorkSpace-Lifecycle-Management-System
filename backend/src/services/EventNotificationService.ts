import { eventBus, Events, EventType } from '../events/eventBus.js'
...
interface NotificationTemplate {
  ...
  priority: 'low' | 'medium' | 'high' | 'urgent' | ((data: any) => 'low' | 'medium' | 'high' | 'urgent')
...
const NOTIFICATION_TEMPLATES: Partial<Record<EventType, NotificationTemplate>> = {
  ...
  [Events.ATTENDANCE_MARKED]: {
    ...
    type: 'attendance',
    priority: (d: any) => (d.attendancePct !== undefined && d.attendancePct < 75 ? 'urgent' : 'low') as any,
...
export class EventNotificationService {
  ...
  private registerListeners() {
    for (const [event, template] of Object.entries(NOTIFICATION_TEMPLATES)) {
      ...
      eventBus.on(event, async (data: any) => {
        try {
          await this.handleEvent(event as EventType, template, data)
        } catch (err: any) {
    ...
    eventBus.on(Events.ATTENDANCE_MARKED, async (data: any) => {
  ...
  private async handleEvent(event: EventType, template: NotificationTemplate, data: any) {
  ...
  private async notifyParentOfAttendanceShortage(data: any) {
    const student = await User.findById(data.userId).select('name parentId').lean()
    if (!student) return

    let parentId: string | null = (student as any).parentId?.toString() || null
    ...
    if (!parentId) {
      ...
      if (parentUser) parentId = parentUser._id.toString()
    }

    if (!parentId) return

    const message = `⚠️ Attendance Alert: ${(student as any).name}'s attendance in ${data.subject || 'class'} has dropped to ${data.attendancePct}%. Please ensure they attend more classes to avoid exam ineligibility.`
  ...
  private async sendEmailNotification(userId: string, subject: string, body: string) {
    ...
    try {
      ...
      // @ts-ignore
      const nodemailerModule: any = await (import('nodemailer') as Promise<any>).catch(() => null)
      if (!nodemailerModule) return
      ...
      logger.info({ userId, email: (user as any).email, subject }, '[EventNotificationService] Email sent')
    } catch (err: any) {
