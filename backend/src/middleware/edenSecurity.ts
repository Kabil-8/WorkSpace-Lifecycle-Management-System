...
const localRequestCounts: Map<string, { count: number; resetTime: number }> = new Map()
 ...
 */
export async function edenRateLimiter(req: Request, res: Response, next: NextFunction) {
  const identifier = (req as any).user?._id?.toString() || req.ip || 'anonymous'
 ...
 */
export function edenPromptSanitizer(req: Request, res: Response, next: NextFunction) {
