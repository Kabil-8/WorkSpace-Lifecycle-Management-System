...
async function runToolAuthorizationTest() {
  ...
  try {
    ...
    const res1 = await ToolRegistry.executeTool(
      'get_at_risk_students',
      {},
      { userId: dummyStudentId, role: 'student', userName: 'Student A' }
    )
    ...
    const res2 = await ToolRegistry.executeTool(
      'get_at_risk_students',
      {},
      { userId: dummyFacultyId, role: 'faculty', userName: 'Dr. Smith', department: 'Computer Science' }
    )
    ...
    const res3 = await ToolRegistry.executeTool(
      'search_eligible_students',
      { minCGPA: 8.0 },
      { userId: dummyFacultyId, role: 'faculty', userName: 'Dr. Smith' }
    )
    ...
    const res4 = await ToolRegistry.executeTool(
      'search_eligible_students',
      { minCGPA: 8.0 },
      { userId: dummyRecruiterId, role: 'recruiter', userName: 'Recruiter Jane' }
    )
    ...
    const res5 = await ToolRegistry.executeTool(
      'get_linked_student_attendance',
      {},
      { userId: dummyParentId, role: 'parent', userName: 'Parent Bob' }
    )
