...
async function runTests() {
  ...
  const reqA: any = { user: { _id: studentA, role: 'student' }, params: { courseId: testCollegeCourse._id.toString() } }
  let resDataA: any = null
  const mockResA: any = {
    json: (d: any) => { resDataA = d },
  ...
  const reqB: any = { user: { _id: studentB, role: 'student' }, params: { courseId: testCollegeCourse._id.toString() } }
  let resDataB: any = null
  let statusCodeB = 200
  const mockResB: any = {
    json: (d: any) => { resDataB = d },
    status: function(code: number) { statusCodeB = code; return this }
  }
  ...
  const reqPublic: any = { query: {} }
  let publicRes: any = null
  const mockResPub: any = { json: (d: any) => { publicRes = d } }
  ...
  assert(publicRes?.data?.courses?.some((c: any) => c.slug === 'test-vc-kubernetes-mastery'), 'Public catalog contains published video courses')
  ...
  let progResA: any = null
  const mockResProgA: any = { json: (d: any) => { progResA = d } }
  ...
  const reqProgBGet: any = {
    user: { _id: studentB, name: 'Student B' },
    params: { courseSlug: 'test-vc-kubernetes-mastery' }
  }
  let progResBGet: any = null
  const mockResProgBGet: any = { json: (d: any) => { progResBGet = d } }
  ...
  let certResA: any = null
  const mockResCertA: any = { json: (d: any) => { certResA = d } }
  ...
  let certResB: any = null
  let certStatusB = 200
  const mockResCertB: any = {
    json: (d: any) => { certResB = d },
    status: function(code: number) { certStatusB = code; return this }
  }
