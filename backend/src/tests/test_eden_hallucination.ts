...
async function runHallucinationTest() {
  ...
  try {
    ...
    const freshStudent = await User.findOneAndUpdate(
    ...
    const attResult = await ToolRegistry.executeTool(
      'get_my_attendance',
      {},
      { userId: freshStudent._id.toString(), role: 'student', userName: freshStudent.name, department: freshStudent.department }
    )
    ...
    const pendingResult = await ToolRegistry.executeTool(
      'get_pending_submissions',
      {},
      { userId: new mongoose.Types.ObjectId().toString(), role: 'faculty', userName: 'Dr. Test', department: 'Data Science & AI' }
    )
