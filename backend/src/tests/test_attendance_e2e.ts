...
async function runAttendanceE2ETest() {
  ...
  try {
    ...
    const parentA = await User.findOneAndUpdate(
    ...
    const toolResult = await ToolRegistry.executeTool(
      'get_my_attendance',
      {},
      { userId: studentA._id.toString(), role: 'student', userName: studentA.name, department: studentA.department }
    )
