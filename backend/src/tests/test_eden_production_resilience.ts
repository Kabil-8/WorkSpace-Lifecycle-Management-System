...
async function runProductionResilienceTest() {
  ...
  try {
    ...
    const res1 = await ToolRegistry.executeTool(
      ...
      {},
      { userId: new mongoose.Types.ObjectId().toString(), role: 'student', userName: 'Student Test' }
    )
    ...
    const res2 = await ToolRegistry.executeTool(
      'get_my_attendance',
      {},
      { userId: new mongoose.Types.ObjectId().toString(), role: 'student', userName: 'Fresh Student' }
    )
    ...
    const res5 = await ToolRegistry.executeTool(
      ...
      { minCGPA: 'not-a-number', department: { $gt: '' } }, // Mongo injection payload attempt
    ...
    const res10 = await ToolRegistry.executeTool(
      'get_my_profile',
      { userId: spoofUserId }, // Spoofed userId in args
