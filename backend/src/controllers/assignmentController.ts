...
export class AssignmentController {
  static async getAll(req: any, res: Response) {
    try {
      const { department, status, courseName, search, priority, page = '1', limit = '50' } = req.query as any
      ...
      const augmented = assignments.map((a: any) => {
        const mySub = (a.submissions || []).find((s: any) => s.studentId?.toString() === userIdStr)
      ...
      if (status && status !== 'all') {
        result = augmented.filter((a: any) => a.status === status)
      }
  ...
  static async submit(req: any, res: Response) {
    try {
      ...
      if (existingSubIndex >= 0) {
        ...
        assignment.submissions.push(submissionPayload as any)
      }
  ...
  static async grade(req: any, res: Response) {
    try {
