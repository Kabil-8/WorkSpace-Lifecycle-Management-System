...
async function extractGitHubStats(username: string) {
...
async function extractLeetCodeStats(username: string) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (matched) {
        ...
        const allItem = acList.find((i: any) => i.difficulty === 'All') || {}
        const easyItem = acList.find((i: any) => i.difficulty === 'Easy') || {}
        const medItem = acList.find((i: any) => i.difficulty === 'Medium') || {}
        const hardItem = acList.find((i: any) => i.difficulty === 'Hard') || {}
...
export async function getCodingStats(req: AuthenticatedRequest, res: Response) {
  try {
...
export async function syncExternalProfiles(req: AuthenticatedRequest, res: Response) {
  try {
    const userId = req.user?.id
    if (!userId) {
      return res.status(401).json({ success: false, message: 'Unauthorized' })
    ...
    const linkedinStats = extractLinkedInStats(liTarget, user.skills, {
      ...
      verifiedSkills: Array.isArray(linkedinSkills) ? linkedinSkills : (typeof linkedinSkills === 'string' ? linkedinSkills.split(',').map((s: string) => s.trim()).filter(Boolean) : undefined),
      certifications: Array.isArray(linkedinCertifications) ? linkedinCertifications : (typeof linkedinCertifications === 'string' ? linkedinCertifications.split(',').map((s: string) => s.trim()).filter(Boolean) : undefined),
    }, user.name)

    // Calculate unified developer score
    const totalScore = Math.round(
      (leetcodeStats.totalSolved * 1.8) +
    ...
    await StudentDigitalTwin.findOneAndUpdate(
    ...
    await Gamification.findOneAndUpdate(
      { userId },
      { $inc: { xp: 100 }, $set: { level: user.level } },
      { upsert: true }
    )
...
export async function getCodingLeaderboard(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    const rankedList = students.map((std: any, index: number) => {
...
export async function syncAtsToProfile(req: AuthenticatedRequest, res: Response) {
  try {
    ...
    if (atsScore !== undefined) {
      ...
      if (!isNaN(parsed) && parsed >= 0 && parsed <= 100) {
        ...
        (user as any).atsScore = safeAtsScore
    ...
    let digitalTwinSyncStatus: 'completed' | 'failed' = 'completed'
    let digitalTwinError: string | undefined
    try {
