...
export class TimetableController {
   ...
   */
  static async getTimetable(req: any, res: Response) {
    try {
      const isStudent = req.user.role === 'student'
      const department = isStudent ? (req.user.department || 'Computer Science') : ((req.query.department as string) || req.user.department || 'Computer Science')
      const semester = parseInt((req.query.semester as string) || (req.user.semester ? String(req.user.semester) : '7'), 10)
      // Students MUST view ONLY their own assigned section
      const section = isStudent ? (req.user.section || 'Section A') : ((req.query.section as string) || req.user.section || 'Section A')
   ...
   */
  static async createPeriodSlot(req: any, res: Response) {
    try {
      const {
        department,
        semester,
   ...
   */
  static async updatePeriodSlot(req: any, res: Response) {
    try {
      const { id } = req.params
   ...
   */
  static async deletePeriodSlot(req: any, res: Response) {
    try {
      const { id } = req.params
