...
export const getDashboard = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (role === 'student') {
      ...
      const gpaHistory = academicResults.length > 0
        ? academicResults.map((r: any) => {
            const semCredits = (r.courseGrades || []).reduce((sum: number, g: any) => sum + (g.credits || 4), 0) || r.totalCreditsEarned || 15
      ...
      dashboardData = {
        ...
        leaderboard: leaderboard.map((e, i) => ({
          ...
          user: e.userId,
          isCurrentUser: (e.userId as any)?._id?.toString() === userId,
        })),
        edenInsights: [
