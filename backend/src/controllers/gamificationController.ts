...
export class GamificationController {
  ...
  static async getLeaderboard(req: Request, res: Response) {
    try {
      const cached = await RedisCache.get<any>(CacheKeys.leaderboard())
  ...
  static async completeMission(req: any, res: Response) {
    try {
      const { missionId } = req.body
      const profile = await Gamification.findOne({ userId: req.user._id })
      if (!profile) return res.status(404).json({ success: false, message: 'Gamification profile not found' })

      const mission = profile.dailyMissions.find((m: any) => m.id === missionId)
      if (!mission) return res.status(404).json({ success: false, message: 'Mission not found' })
      if (mission.isCompleted) return res.status(400).json({ success: false, message: 'Mission already completed' })
      ...
      if (profile.xp >= xpForNextLevel) profile.level += 1
  ...
  static async awardXp(req: any, res: Response) {
    try {
      const { userId, amount, reason } = req.body
      await Gamification.findOneAndUpdate(
        { userId },
        { $inc: { xp: amount, weeklyXp: amount, monthlyXp: amount } },
        { upsert: true }
      )
