...
export class CollegeCourseController {
   ...
   */
  static async getMyCollegeCourses(req: any, res: Response) {
    try {
   ...
   */
  static async getCollegeCourseById(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
      ...
      if (!course) return res.status(404).json({ success: false, message: 'College course not found.' })
   ...
   */
  static async getCourseAssignments(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async submitAssignment(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
      ...
      if (!submissionText) return res.status(400).json({ success: false, message: 'submissionText required' })

      const sub = await CollegeAssignmentSubmission.findOneAndUpdate(
        { assignmentId: new mongoose.Types.ObjectId(assignmentId), studentId: uId },
        { submissionText, submittedAt: new Date(), status: 'submitted' },
   ...
   */
  static async getCourseQuizzes(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async submitQuiz(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async getCourseExams(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async getCoursePerformance(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async createCollegeCourse(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
   ...
   */
  static async updateCollegeCourse(req: any, res: Response) {
    try {
      const { courseId } = req.params
   ...
   */
  static async deleteCollegeCourse(req: any, res: Response) {
    try {
      const { courseId } = req.params
   ...
   */
  static async enrollInCourse(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
