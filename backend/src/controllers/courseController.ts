...
export class CourseController {
  static async getAll(req: Request, res: Response) {
    try {
      const { department, level, search, page = '1', limit = '20' } = req.query as any
   ...
   */
  static async updateUserProgress(req: any, res: Response) {
    try {
      const courseId = req.params.id
      const userId = req.user._id
   ...
   */
  static async addMaterial(req: any, res: Response) {
    try {
      const courseId = req.params.id
