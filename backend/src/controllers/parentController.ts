...
export class ParentController {
   ...
   */
  static async getLinkedStudents(req: any, res: Response) {
    try {
      ...
      const studentIds: any[] = [
        ...
        ...((parent as any)?.linkedStudentId ? [(parent as any).linkedStudentId] : []),
      ]
   ...
   */
  static async getLinkedStudentAttendance(req: any, res: Response) {
    try {
   ...
   */
  static async getLinkedStudentAssignments(req: any, res: Response) {
    try {
      const { studentId } = req.params
      const authorized = await ParentController.isAuthorizedForStudent(req.user._id, studentId)
      if (!authorized) {
      ...
      const student = await User.findById(studentId).select('name department').lean()
      const assignments = await Assignment.find({ department: (student as any)?.department })
   ...
   */
  static async getLinkedStudentCourses(req: any, res: Response) {
    try {
      const { studentId } = req.params
      const authorized = await ParentController.isAuthorizedForStudent(req.user._id, studentId)
      if (!authorized) {
   ...
   */
  private static async isAuthorizedForStudent(parentId: string, studentId: string): Promise<boolean> {
    try {
      ...
      if ((parent as any).role === 'admin' || (parent as any).role === 'super_admin') return true

      const linked: string[] = [
        ...((parent as any).linkedStudentIds?.map((id: any) => id.toString()) || []),
        ...((parent as any).linkedStudentId ? [(parent as any).linkedStudentId.toString()] : []),
      ]
