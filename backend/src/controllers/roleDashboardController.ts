...
export class RoleDashboardController {
  public static async getRoleDashboard(req: Request, res: Response): Promise<void> {
    try {
      ...
      switch (role) {
        case 'student': {
          ...
          const presentAtt = myAttendance.filter((a: any) => a.status === 'present' || a.status === 'Present').length
      ...
      res.status(500).json({ success: false, message: 'Failed to load dashboard telemetry', error: (err as Error).message })
    }
  }

  public static getStudentDashboard = RoleDashboardController.getRoleDashboard
  public static getFacultyDashboard = RoleDashboardController.getRoleDashboard
}
