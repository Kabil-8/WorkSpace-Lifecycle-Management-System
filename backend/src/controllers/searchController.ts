...
export class SearchController {
  static async globalSearch(req: Request, res: Response) {
    try {
      const { q, types = 'all', limit = '5' } = req.query as any
      ...
      if (searchAll || typeList.includes('users')) {
        searches.push(
          User.find({ $text: { $search: q }, isActive: true })
      ...
      if (searchAll || typeList.includes('courses')) {
        searches.push(
          Course.find({ $text: { $search: q }, status: 'published' })
      ...
      if (searchAll || typeList.includes('jobs')) {
        searches.push(
          Job.find({ $text: { $search: q }, isActive: true })
