...
export class VideoCourseController {
   ...
   */
  static async getCatalog(req: any, res: Response) {
    try {
   ...
   */
  static async getCourseBySlug(req: any, res: Response) {
    try {
      const { courseSlug } = req.params
      const course = await VideoCourse.findOne({ slug: courseSlug, published: true }).lean()
      if (!course) return res.status(404).json({ success: false, message: 'Video course not found' })
   ...
   */
  static async getLessonWatchDetails(req: any, res: Response) {
    try {
   ...
   */
  static async getCertificate(req: any, res: Response) {
    try {
      const uId = getUid(req)
      if (!uId) return res.status(401).json({ success: false, message: 'Unauthorized' })
      ...
      if (!certificate) {
        ...
        certificate = newCert.toObject() as any
      }
