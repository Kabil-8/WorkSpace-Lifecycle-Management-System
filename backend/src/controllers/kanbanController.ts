...
export class KanbanController {
  static async getBoard(req: any, res: Response) {
    try {
      const user = req.user
  ...
  static async createTask(req: any, res: Response) {
    try {
      ...
      board.tasks.push(newTask as any)
      await board.save()
  ...
  static async updateTask(req: any, res: Response) {
    try {
  ...
  static async deleteTask(req: any, res: Response) {
    try {
      const { taskId } = req.params
      const board = await KanbanBoard.findOne({ 'tasks.id': taskId })
      if (!board) {
