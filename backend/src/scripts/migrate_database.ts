...
async function migrateDatabase() {
  ...
  try {
    ...
    if (!oldConn.db) throw new Error('Failed to initialize source database connection.')
    ...
    if (!newConn.db) throw new Error('Failed to initialize target database connection.')
