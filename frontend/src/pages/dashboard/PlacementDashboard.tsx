...
import { Building2, Star, Loader2, Info } from 'lucide-react'
...
export default function PlacementDashboard() {
  ...
  const { data: placementRes, isLoading } = useQuery({
    queryKey: ['placementDashboard'],
    queryFn: async () => {
      const res: any = await api.get('/dashboard/placement_officer')
      return res?.data || res
    },
  })
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-xs text-slate-400">Loading Placement Cell Telemetry...</p>
        </div>
      </div>
    )
  }

  const d = placementRes || {}
  const stats = d.stats || []
  ...
  return (
    <div className="page-container space-y-5">
      ...
      <motion.div
        ...
        style={{ background: 'linear-gradient(135deg, #051a24 0%, #0a3348 60%, #0d4663 100%)', border: '1px solid rgba(6,182,212,0.25)' }}
      >
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-2xs text-cyan-400 font-mono">Live Institutional DB</span>
            </div>
            <h1 className="text-2xl font-bold text-white">Placement Officer Dashboard</h1>
            <p className="text-sm mt-1" style={{ color: 'rgba(255,255,255,0.6)' }}>
              Campus recruitment drives, company hiring funnels, and student ATS readiness
            </p>
          </div>
          <div className="flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-cyan-400 animate-ping" />
            <span className="text-xs font-mono text-cyan-300">Live Drive Telemetry</span>
          </div>
        </div>
      </motion.div>
      ...
      {/* Drive Progress Chart */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="stat-card lg:col-span-2">
          <h3 className="font-semibold mb-4 text-white">Placement Drive Progress (By Phase)</h3>
          {chartData.length > 0 ? (
            ...
            <div className="flex items-center justify-center py-12 text-slate-400 text-xs">
              <Info size={16} className="mr-2" /> No placement drives recorded yet.
            </div>
          )}
        </div>
        ...
        <div className="stat-card space-y-4">
          <h3 className="font-semibold text-white flex items-center gap-2">
            <Building2 size={16} className="text-cyan-400" /> Placement Cell Highlights
          </h3>
